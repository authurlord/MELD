...
def convert_alpaca_ids(
    ...
    Converts alpaca format dataset to the standard format.
    """
    ids_value = example.get("ids", None)
    prompt = []
    if dataset_attr.history and isinstance(example[dataset_attr.history], list):
        for old_prompt, old_response in example[dataset_attr.history]:
            prompt.append({"role": Role.USER.value, "content": old_prompt})
...
## convert sharegpt with ids
def convert_sharegpt_ids( 
    example: Dict[str, Any],
    dataset_attr: "DatasetAttr",
    data_args: "DataArguments",
) -> Dict[str, Any]:
    ...
    Converts sharegpt format dataset to the standard format.
    """
    ids_value = example.get("ids", None) ## get id value
    tag_mapping = {
        dataset_attr.user_tag: Role.USER.value,
        dataset_attr.assistant_tag: Role.ASSISTANT.value,
        dataset_attr.observation_tag: Role.OBSERVATION.value,
...
def align_dataset(
    ...
    if dataset_attr.formatting == "alpaca" and 'ids' in column_names:
        convert_func = partial(convert_alpaca_ids, dataset_attr=dataset_attr, data_args=data_args)
        print('Hooked! transfer alpaca to alpaca_ids')
    elif dataset_attr.formatting == "alpaca":
        convert_func = partial(convert_alpaca, dataset_attr=dataset_attr, data_args=data_args)
    elif 'ids'.isin(column_names):
        convert_func = partial(convert_sharegpt_ids, dataset_attr=dataset_attr, data_args=data_args)
        print('Hooked! transfer sharegpt to sharegpt_ids')
    else:
    ...
    # print(column_names)
    column_names = [name for name in column_names if name!= 'ids'] ## omit ids
    return dataset.map(
        convert_func,
        batched=False,
